...
// ── ROBOFLOW config ───────────────────────────────────────────────────────────
const ROBOFLOW_API_KEY = 'Q2aZUDuIYU4yyoDhXXQg';
const ROBOFLOW_ENDPOINT =
  'https://serverless.roboflow.com/aidans-workspace-52o3b/workflows/detect-count-and-visualize';
...
// ── FIX: satellite URL was blank in original file, causing a syntax error ─────
...
async function overpassFetch(query, callerSignal, timeoutMs = 25000) {
  ...
  const racePromises = OVERPASS_ENDPOINTS.map((endpoint, i) => {
    ...
    return fetch(endpoint, {
      ...
      clearTimeout(timer);
      if (err.name !== 'AbortError') log(`✗ ${host}: ${err.message}`, 'error');
      throw err;
    });
  });
...
async function calculateHvac(osmId) {
  ...
  try {
    ...
    // Log full response — inspect exact key names in DevTools → Console
    console.log('[Roboflow raw response]', JSON.stringify(result, null, 2));

    const output = result?.outputs?.[0] ?? {};

    // Broaden count extraction — covers common Roboflow workflow output shapes
...
function renderBuildings(data, highlightMode = 'all', searchLat = null, searchLng = null) {
  ...
  for (const el of data.elements) {
    ...
    polygon.on('click', function () { handleBuildingClick(this); });

    polygon.on('mouseover', function () {
      const mode = document.getElementById('highlight-mode').value;
      if (mode === 'one' && this === selectedLayer) return;
    ...
    polygon.on('mouseout', function () {
      const mode = document.getElementById('highlight-mode').value;
      if (mode === 'one' && this === selectedLayer) return;
      this.setStyle(mode === 'one' ? dimmedStyle : buildingStyle);
    });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ── Ping indicator ────────────────────────────────────────────────────────
  // FIX: HEAD requests to overpass-api.de are blocked by CORS in browsers.
  // Use a POST with a trivial query instead — same endpoint your searches use.
  ...
  async function runPing() {
    ...
    const t0 = performance.now();
    try {
      // Ping a lightweight CORS-friendly endpoint — avoids hammering Overpass quota
  ...
  runPing();
  setInterval(runPing, 30000); // every 30s is plenty
});
